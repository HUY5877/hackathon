...
Vercel 已在启动脚本中通过 Alembic 建立过两次数据库连接，因此应用 lifespan 在 `VERCEL=1` 时跳过重复的 `SELECT 1` 探测；普通服务器和 Docker Compose 不受影响，调度器等其他 lifespan 初始化仍正常执行。
...
- `app/main.py` 仅对 Vercel 跳过重复的启动数据库探测。
